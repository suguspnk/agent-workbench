...
EXPECTED_ROLES = {
    ...
    "awb_ownership_probe": ("gpt-5.6-luna", "low", "read-only"),
...
CLAUDE_PROFILES = {
    ...
    "awb-ownership-probe": ("haiku", "low", True),
...
CODEX_PROFILES: dict[str, tuple[Any, ...]] = {
    ...
    "awb_ownership_probe": ("awb_ownership_probe", "Deprecated compatibility profile; runtime ownership probing is lead-owned protected MCP only.", "gpt-5.6-luna", "low", "read-only", "cd7d62d7ed26c877b507680ccdaff8fb5dc99345550d46fa4b095acb99f31c6a"),
...
CLAUDE_PROFILE_TUPLES: dict[str, tuple[Any, ...]] = {
    ...
    "awb-ownership-probe": ("awb-ownership-probe", "Deprecated compatibility profile; runtime ownership probing is lead-owned protected MCP only.", "haiku", "low", frozenset({"Glob"}), "65282ea7e8c1b3a9481c64a0655a8a7493cf8ec75c4c0f2757db958a0e3df9af"),
...
OWNERSHIP_PROBE_REGISTRY_DESCRIPTOR = {'version': 6,
 ...
 'adapter_result_schema': {'required_fields': ['adapter_result_version',
                                               'descriptor_sha256',
                                               'descriptor_version',
                                               'query_results',
                                               'tool_name',
 ...
 'parent_context_schema': {'version': 2,
                           'required_fields': ['adapter_result',
                                               'context_version',
 ...
 'tool_constraints': {'mcp_server': 'awb_ownership',
                      ...
                      'forbidden': ['ownership-probe-child',
                                    ...
                                    'implementation-governance',
                                    'symlink-following']},
 'lifecycle': {'work_cutoff_seconds': 45,
...
OWNERSHIP_PROBE_CONTRACT = {'artifact_classes': ['ecs-task-definition-manifests', 'deployment-pipeline-manifests', 'infrastructure-as-code'],
 'execution_owner': 'lead-owned-protected-mcp',
 'forbidden': ['caller-supplied-input',
...
def validate_ownership_probe_descriptor(value: object) -> None:
    if type(value) is not dict or value != OWNERSHIP_PROBE_REGISTRY_DESCRIPTOR:
        fail("router ownership probe descriptor differs from the protected portable registry")
...
PLANNER_LIFECYCLE_CONTRACT = {
    ...
    "direct_diagnosis": DIRECT_DIAGNOSIS_CONTRACT,
    ...
    "ownership_probe": OWNERSHIP_PROBE_CONTRACT,
...
PLANNER_LIFECYCLE_SKILL_REQUIREMENTS = (
    ...
    "Neither the identity preflight nor the probe relaxes eventual implementation test, verifier, reviewer, or security-review overlays",
    ...
    "full adapter result",
    "never trusts a tool-supplied outcome or task criterion",
    "Use one MCP call, zero children, zero waits",
    "Missing, malformed, stale, replayed, mixed-version",
    "90-second work cutoff, 120-second hard deadline, 30-second handoff reserve",
...
def validate_runtime_ownership_probe_surfaces(
    ...
    for label, section in sections.items():
        ...
        for stale in ("send exactly one `awb_ownership_probe`", "claude `glob` only"):
            if stale in section.lower():
                fail(f"{label} runtime ownership probe retains a child or Glob execution path")
...
def validate_ownership_probe_profile_contract(body: str, label: str) -> None:
    for phrase in (
        ...
        "Do not spawn it",
        "awb_ownership.scan_required_artifacts",
        "return `inconclusive-delegate` immediately",
        "zero children, waits, commands, file reads, network, credentials, writes, tests, or governance loads",
...
def check_replays_and_unit_tests() -> None:
    ...
    expected_descriptor_digest = hashlib.sha256(canonical_descriptor).hexdigest()
    descriptor_receipt = f"ownership probe descriptor sha256 {expected_descriptor_digest}"
    if result.stdout.startswith("Routing replay passed") and result.stdout.count(descriptor_receipt) != 1:
        fail("routing replay ownership probe descriptor differs from the protected portable registry")
