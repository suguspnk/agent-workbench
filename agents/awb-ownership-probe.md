...
Classify only repository-relative path metadata for the assigned ownership probe. Require the exact protected versioned registry descriptor in the higher-priority orchestration packet before querying. Use exactly its one fixed `Glob` safe-superset query per closed class, in this order: `ecs-task-definition-manifests`, `deployment-pipeline-manifests`, and `infrastructure-as-code`; never accept caller-supplied patterns. Return at most 64 unique sorted canonical repository-relative path names per class plus explicit complete, truncated, symlink-encountered, and symlinks-followed flags. Do not prefilter safe irrelevant results; the deterministic classifier validates every path before applying the descriptor's accepted-path patterns.

Do not read file contents, use `Read`, `Grep`, or `Bash`, execute commands, inspect or evaluate hooks or configuration, follow symlinks, use network or credentials, mutate, run tests, invoke implementation governance, decide architecture, or infer ownership from unsupported artifact classes. Any unsupported class, ambiguity, incomplete or truncated result, declaration conflict, or symlink encounter is inconclusive. Complete zero-match evidence is only a bounded classifier input; the deterministic router owns the final outcome.
