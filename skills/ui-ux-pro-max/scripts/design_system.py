...
Usage:
    ...
    result = generate_design_system(
        "SaaS dashboard", "My Project", persist=True,
        output_dir="/path/to/project", confirm_write=True,
    )
    result["persistence"]  # {"status": "success"|"skipped_exists", "created_files": [...], ...}
    result = generate_design_system(
        "SaaS dashboard", "My Project", persist=True, page="dashboard",
        output_dir="/path/to/project", confirm_write=True,
    )
"""
...
def generate_design_system(query: str, project_name: str = None, output_format: str = "ascii",
                           ...
                           force: bool = False, confirm_write: bool = False) -> dict:
    """
    ...
    Args:
        ...
        output_dir: Required absolute project root when persist is True
        ...
        confirm_write: Must be True when persist is True, asserting that the user
                       explicitly authorized writing project files
...
class _PersistenceTargetExists(Exception):
    """Internal signal for a no-overwrite collision after preflight."""


class _PersistenceBindingChanged(ValueError):
    """The retained directory chain is no longer rooted at the authorized path."""


def _require_secure_persistence_support() -> None:
    ...
    if any(function not in os.supports_dir_fd for function in required_dir_fd):
        missing.append("descriptor-relative filesystem operations")
    if os.stat not in os.supports_follow_symlinks or os.link not in os.supports_follow_symlinks:
        missing.append("no-follow filesystem operations")
    if missing:
        raise ValueError(
            "secure persistence is unsupported on this platform: " + ", ".join(missing)
        )
...
def _lstat_at(parent_fd: int, name: str):
    """Return an entry's stat without following it, or None when absent."""
    try:
...
def _raise_unsafe_entry(display_path: Path, entry_stat) -> None:
    if stat.S_ISLNK(entry_stat.st_mode):
        raise ValueError(f"persistence target contains a symbolic link: {display_path}")
    if not stat.S_ISDIR(entry_stat.st_mode):
        raise ValueError(f"persistence path component is not a directory: {display_path}")


def _open_directory_at(parent_fd: int, name: str, display_path: Path,
                       create: bool = False) -> tuple:
    """Open one directory component without following links, optionally creating it."""
    entry_stat = _lstat_at(parent_fd, name)
    created = False
    if entry_stat is None and create:
        try:
            os.mkdir(name, mode=0o777, dir_fd=parent_fd)
            created = True
        except FileExistsError:
            # Another actor won the create race. The no-follow open and identity
            # comparison below decide whether the new entry is safe.
            pass
        entry_stat = _lstat_at(parent_fd, name)

    if entry_stat is None:
        if create:
            raise ValueError(f"persistence directory changed during creation: {display_path}")
        return None, False
    _raise_unsafe_entry(display_path, entry_stat)

    try:
        child_fd = os.open(name, _DIRECTORY_OPEN_FLAGS, dir_fd=parent_fd)
    except OSError as error:
        raced_stat = _lstat_at(parent_fd, name)
        if raced_stat is not None and stat.S_ISLNK(raced_stat.st_mode):
            raise ValueError(
                f"persistence target contains a symbolic link: {display_path}"
            ) from error
        raise ValueError(f"could not securely open persistence directory: {display_path}") from error

    opened_stat = os.fstat(child_fd)
    bound_stat = _lstat_at(parent_fd, name)
    if bound_stat is None or not stat.S_ISDIR(bound_stat.st_mode) or not _same_file(opened_stat, bound_stat):
        os.close(child_fd)
        raise ValueError(f"persistence directory changed while it was being opened: {display_path}")
    return child_fd, created


def _target_stat(base_fd: int, directory_parts: tuple, filename: str,
                 base_dir: Path):
    """Securely inspect a target through existing directory descriptors."""
    parent_fd = os.dup(base_fd)
    display_path = base_dir
    try:
        for part in directory_parts:
            display_path = display_path / part
            child_fd, _ = _open_directory_at(parent_fd, part, display_path, create=False)
            if child_fd is None:
                return None
            os.close(parent_fd)
            parent_fd = child_fd
        target_stat = _lstat_at(parent_fd, filename)
        if target_stat is not None and stat.S_ISLNK(target_stat.st_mode):
            raise ValueError(
                f"persistence target contains a symbolic link: {display_path / filename}"
            )
        return target_stat
    finally:
        os.close(parent_fd)
...
def _open_directory_chain(base_fd: int, parts: tuple, base_dir: Path) -> tuple:
    """Securely create/open a directory chain and retain its binding metadata."""
    descriptors = []
    parent_fd = os.dup(base_fd)
    display_path = base_dir
    try:
        for part in parts:
            display_path = display_path / part
            child_fd, created = _open_directory_at(parent_fd, part, display_path, create=True)
            descriptors.append((parent_fd, part, child_fd, display_path, created))
            parent_fd = child_fd
        return descriptors
    except Exception:
        os.close(parent_fd)
...
def _verify_directory_chain(descriptors: list) -> None:
    """Confirm each retained directory descriptor is still bound to its path entry."""
    for parent_fd, name, child_fd, display_path, _ in descriptors:
        bound_stat = _lstat_at(parent_fd, name)
        opened_stat = os.fstat(child_fd)
        if (
            bound_stat is None
            or not stat.S_ISDIR(bound_stat.st_mode)
            or not _same_file(opened_stat, bound_stat)
        ):
            raise _PersistenceBindingChanged(
                f"persistence directory changed during writing: {display_path}"
            )


def _close_directory_chain(descriptors: list) -> None:
    if not descriptors:
        return
    os.close(descriptors[-1][2])
    for parent_fd, _, _, _, _ in reversed(descriptors):
        os.close(parent_fd)


def _rollback_created_file(parent_fd: int, filename: str, identity: tuple,
                           display_path: Path, mutation_guard=None) -> None:
    """Remove only the exact inode linked by this no-overwrite invocation."""
    if mutation_guard is not None:
        mutation_guard()
    current_stat = _lstat_at(parent_fd, filename)
    if current_stat is None:
        raise ValueError(
            f"persistence target disappeared before safe rollback; inspect: {display_path}"
        )
    if (current_stat.st_dev, current_stat.st_ino) != identity:
        raise ValueError(
            f"persistence target changed before safe rollback; it was left untouched: {display_path}"
        )
    if mutation_guard is not None:
        mutation_guard()
...
def _atomic_write_at(parent_fd: int, filename: str, content: str,
                     display_path: Path, force: bool,
                     mutation_guard=None) -> tuple:
    """Atomically create or replace one regular file in a retained directory."""
    ...
    temp_created = False
    post_link_rollback_completed = False
    try:
        if mutation_guard is not None:
            mutation_guard()
        temp_fd = os.open(temp_name, temp_flags, 0o666, dir_fd=parent_fd)
        temp_created = True
        with os.fdopen(temp_fd, "w", encoding="utf-8", newline="") as handle:
            handle.write(content)
            handle.flush()
            os.fsync(handle.fileno())
        staged_stat = _lstat_at(parent_fd, temp_name)
        if staged_stat is None or not stat.S_ISREG(staged_stat.st_mode):
            raise ValueError(f"persistence staging file is not a regular file: {display_path}")
        ...
        if force:
            if current_stat is not None and not stat.S_ISREG(current_stat.st_mode):
                raise ValueError(f"persistence target exists but is not a regular file: {display_path}")
            try:
                if mutation_guard is not None:
                    mutation_guard()
                os.replace(
                ...
                raise ValueError(
                    "secure persistence is unsupported on this platform: descriptor-relative replace"
                ) from error
            temp_created = False
        else:
            try:
                ...
                raise _PersistenceTargetExists(str(display_path)) from error

        if force:
            committed_stat = _lstat_at(parent_fd, filename)
            if (
                committed_stat is None
                or not stat.S_ISREG(committed_stat.st_mode)
                or not _same_file(staged_stat, committed_stat)
            ):
                raise ValueError(
                    f"persistence target changed while it was being committed: {display_path}"
                )
        else:
            try:
                committed_stat = _lstat_at(parent_fd, filename)
            ...
            except Exception as post_link_error:
                try:
                    ...
                    raise ValueError(
                        "no-overwrite persistence post-link handling failed and the "
                        "exact final file could not be rolled back safely; the final file "
                        "may remain committed. "
                        f"Rollback failure: {rollback_error}. Inspect the final target "
                        f"and staging file: {display_path}"
                    ) from rollback_error
                post_link_rollback_completed = True
                raise ValueError(
                    "no-overwrite persistence commit was rolled back after a post-link "
                    f"failure; no final file was committed: {display_path}"
                ) from post_link_error
            temp_created = False
        return staged_stat.st_dev, staged_stat.st_ino
    finally:
        if temp_created:
            active_error = sys.exc_info()[1]
            staging_path = display_path.parent / temp_name
            try:
                if mutation_guard is not None:
                    mutation_guard()
                os.unlink(temp_name, dir_fd=parent_fd)
                ...
                pass
            except _PersistenceBindingChanged as binding_error:
                original_failure = (
                    f" Original failure: {active_error}." if active_error is not None else ""
                )
                raise ValueError(
                    "persistence root binding changed; cleanup through the detached "
                    "directory descriptor was prohibited, so the staging artifact was "
                    f"left orphaned for manual inspection: {staging_path}.{original_failure}"
                ) from binding_error
            except Exception as cleanup_error:
                if post_link_rollback_completed:
                    raise ValueError(
                        "no final file is present, but its staging artifact could not "
                        f"be removed; manual inspection is required: {staging_path}. "
                        f"Staging cleanup failure: {cleanup_error}"
                    ) from cleanup_error
                if active_error is not None:
                    raise ValueError(
                        f"{active_error} Additionally, the staging artifact could not be "
                        f"removed and may remain orphaned: {staging_path}. "
                        f"Staging cleanup failure: {cleanup_error}"
                    ) from active_error
                raise ValueError(
                    "persistence failed and its staging artifact could not be removed; "
                    f"manual inspection is required: {staging_path}"
                ) from cleanup_error
...
def persist_design_system(design_system: dict, page: str = None, output_dir: str = None,
    ...
    Args:
        ...
        page: Optional page name for page-specific override file
        output_dir: Required absolute project root
        page_query: Optional query string for intelligent page override generation
        force: If True, overwrite all existing targets. If False (default) and
               any target already exists, the entire persistence operation is
               skipped so prior design decisions aren't silently discarded.
        confirm_write: Must be True to assert explicit user authorization

    Returns:
        dict with created file paths and status. status is "skipped_exists" if
        any target already existed and force was not set.
    """
    if not confirm_write:
        raise ValueError("persistence requires explicit user authorization (confirm_write=True)")
    if not output_dir:
        raise ValueError("persistence requires an explicit absolute output_dir project root")

    _require_secure_persistence_support()

    requested_base = Path(output_dir).expanduser()
    if not requested_base.is_absolute():
        raise ValueError("output_dir must be an absolute project root")
    if not requested_base.exists() or not requested_base.is_dir():
        raise ValueError("output_dir must identify an existing project directory")
    base_dir = requested_base.resolve(strict=True)
    ...
    targets = [master_file]
    if page:
        targets.append(pages_dir / f"{safe_slug(page, 'page')}.md")

    base_flags = _DIRECTORY_OPEN_FLAGS
    try:
        base_fd = os.open(str(base_dir), base_flags)
    except OSError as error:
        raise ValueError(f"could not securely open output_dir project root: {base_dir}") from error

    # This descriptor-relative preflight is only for all-or-nothing skip behavior
    # and clear errors. Security does not depend on a path check such as
    # current.is_symlink(); every later open and commit is descriptor-relative,
    # no-follow, identity-checked, and atomic.
    try:
        target_specs = [(("design-system", project_slug), "MASTER.md", master_file)]
        if page:
            target_specs.append(
                (("design-system", project_slug, "pages"), f"{safe_slug(page, 'page')}.md", targets[1])
            )
        existing_targets = [
            path
            for directory_parts, filename, path in target_specs
            if _target_stat(base_fd, directory_parts, filename, base_dir) is not None
        ]
        if existing_targets and not force:
            return {
                ...
                "created_files": [],
                "existing_files": [str(path) for path in existing_targets],
                "message": (
                    "One or more persistence targets already exist; no files were modified. "
                    "Read every existing target first, then re-run with force=True / --force "
                    "only after explicit overwrite authorization."
                ),
            }

        if force:
            for directory_parts, filename, path in target_specs:
                target_stat = _target_stat(base_fd, directory_parts, filename, base_dir)
                if target_stat is not None and not stat.S_ISREG(target_stat.st_mode):
                    raise ValueError(f"persistence target exists but is not a regular file: {path}")

        # Format before creating directories. A component changed after preflight
        # is rejected by the secure descriptor walk below, never followed.
        master_content = format_master_md(design_system)
        page_content = format_page_override_md(design_system, page, page_query) if page else None

        chain_parts = ("design-system", project_slug, "pages")
        directory_chain = _open_directory_chain(base_fd, chain_parts, base_dir)
        created_files = []
        committed_files = []
        try:
            _verify_directory_chain(directory_chain)
            mutation_guard = lambda: _verify_directory_chain(directory_chain)
            project_fd = directory_chain[1][2]
            pages_fd = directory_chain[2][2]

            # Recheck all targets through the retained descriptors after formatting
            # and directory creation. A no-overwrite collision skips before any
            # final file is committed.
            if not force:
                late_existing = []
                final_targets = [(project_fd, "MASTER.md", master_file)]
                if page:
                    final_targets.append((pages_fd, targets[1].name, targets[1]))
                for parent_fd, filename, path in final_targets:
                    target_stat = _lstat_at(parent_fd, filename)
                    if target_stat is not None and stat.S_ISLNK(target_stat.st_mode):
                        raise ValueError(f"persistence target contains a symbolic link: {path}")
                    if target_stat is not None:
                        late_existing.append(path)
                if late_existing:
                    return {
                        "status": "skipped_exists",
                        "design_system_dir": str(design_system_dir),
                        "master_file": str(master_file),
                        "created_files": [],
                        "existing_files": [str(path) for path in late_existing],
                        "message": (
                            "A persistence target appeared after preflight; no files were modified. "
                            "Inspect the targets before retrying."
                        ),
                    }

            master_identity = _atomic_write_at(
                project_fd,
                "MASTER.md",
                master_content,
                master_file,
            ...
            committed_files.append((project_fd, "MASTER.md", master_identity, master_file))
            created_files.append(str(master_file))
            if page:
                page_file = targets[1]
                _verify_directory_chain(directory_chain)
                page_identity = _atomic_write_at(
                    pages_fd,
                    page_file.name,
                    page_content,
                    page_file,
                ...
                committed_files.append(
                    (pages_fd, page_file.name, page_identity, page_file)
                )
                created_files.append(str(page_file))
            _verify_directory_chain(directory_chain)
        except _PersistenceTargetExists as error:
            # A target appeared after preflight. Avoid overwriting it and report
            # the same safe skip status used for pre-existing targets.
            for parent_fd, filename, identity, path in reversed(committed_files):
                _rollback_created_file(
                    parent_fd, filename, identity, path, mutation_guard
                )
            return {
                "status": "skipped_exists",
                "design_system_dir": str(design_system_dir),
                "master_file": str(master_file),
                "created_files": [],
                "existing_files": [str(error)],
                "message": (
                    "A persistence target appeared while writing; no existing file was modified. "
                    "Inspect the targets before retrying."
                ),
            }
            ...
            if not force:
                ...
                if rollback_errors:
                    ...
                    raise ValueError(
                        f"{persistence_error} Additionally, one or more earlier final "
                        "files may remain committed because safe rollback failed. "
                        f"Rollback failure: {rollback_details}"
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("")
    lines.append("> **LOGIC:** Read this Master file first. Then check `pages/[page-name].md` relative to this file.")
    lines.append("> If that page file exists, apply its rules as overrides for that page only.")
    lines.append("")
    lines.append("---")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    lines.append("> ⚠️ **IMPORTANT:** Read the Master file (`../MASTER.md`) first, then apply the rules in this file as overrides for this page only.")
