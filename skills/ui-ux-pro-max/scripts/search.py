...
Usage: python search.py "<query>" [--domain <domain>] [--stack <stack>] [--max-results 3]
       ...
       python search.py "<query>" --design-system --persist [-p "Project Name"] --output-dir "/absolute/project-root" --confirm-write (--no-overwrite | --force) [--page "dashboard"]
...
Persistence (Master + Overrides pattern):
  ...
  --output-dir   Absolute project root under which design-system/ is created (required)
  --confirm-write
                 Assert that the user authorized writing project files (required)
  --page         Also create a page-specific override file in design-system/<project-slug>/pages/
  --no-overwrite Skip the entire write if any target already exists
  --force        Overwrite every existing target after explicit user authorization
"""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-dir", "-o", type=str, default=None, help="Absolute project root for persisted files (required with --persist)")
    parser.add_argument("--confirm-write", action="store_true", help="Assert that the user explicitly authorized writing project files")
    overwrite_group = parser.add_mutually_exclusive_group()
    overwrite_group.add_argument("--no-overwrite", action="store_true", help="Skip all persistence if any target already exists")
    overwrite_group.add_argument("--force", action="store_true", help="Overwrite every existing persistence target after explicit authorization")
    ...
    persistence_options = any((args.persist, args.page, args.output_dir, args.confirm_write, args.no_overwrite, args.force))
    if persistence_options and not args.design_system:
        parser.error("persistence options require --design-system")
    if args.persist:
        if not args.output_dir:
            parser.error("--persist requires an explicit --output-dir with an absolute project root")
        if not args.confirm_write:
            parser.error("--persist requires --confirm-write after the user authorizes project-file writes")
        if not (args.no_overwrite or args.force):
            parser.error("--persist requires an explicit overwrite choice: --no-overwrite or --force")
    elif any((args.page, args.output_dir, args.confirm_write, args.no_overwrite, args.force)):
        parser.error("--page, --output-dir, --confirm-write, --no-overwrite, and --force require --persist")
            ...
            if args.persist:
                ...
                if persistence.get("status") == "skipped_exists":
                    ...
                    for f in persistence.get("created_files", []):
                        print(f"   📄 {f}")
                    print("")
                    print(f"📖 Usage: Read {ds_dir}/MASTER.md first.")
                    print("   Then apply pages/[page].md as an override for that page, if it exists.")
                print("=" * 60)
    # Stack search
    elif args.stack:
