...
- External or destructive authority is valid only for one exact action and canonical target in a current trusted-user authorization. Route that packet to the read-only-sandboxed `awb_operator` with packet ID/revision, a stable authorization binding, explicit approval, recovery/irreversibility, and independent verification fields. The binding detects inconsistent packet references; it is not proof of authenticity, so trusted packet transport remains required. Current profiles do not support owned-path deletion and fail closed instead of widening write authority.
- Verify an operator result with a separate `verify-external` card: the same operator packet ID/revision/action/canonical target and binding, exact read-only scope, separate trusted-user authorization, public read-only access, direct observation independent of the operator handoff, and explicit `external-verification` plus network/shell requirements. Ordinary verifier packets remain network-denied.
...
- Before a repository-controlled command, require inspection of its entrypoint and transitive scripts, hooks, plugins, and configuration. Run with network and credentials disabled except for an exact operator packet or the public read-only network scope of an exact external-verification packet; use isolated caches or ephemeral data stores.
...
- Follow-up order is implementation/operator, test when required, direct verifier (using a separately authorized external-verification card after an operator), ordinary reviewer, then security reviewer. Never treat the operator handoff as verification evidence. Any material mutation invalidates all earlier verification and review that could be affected; rerun them on the final tree.
