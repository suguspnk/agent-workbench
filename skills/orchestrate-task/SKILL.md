...
After `inconclusive-delegate` and before ordinary routing, use exactly one `awb_ownership_probe` / `awb-ownership-probe` child when the objective packet asserts that at least one named registered artifact class must exist in the objective-owning repository. This is a path-metadata classifier, not source investigation. It uses the efficient tier at low effort, Codex read-only, and Claude `Glob` only. It must not read file contents; execute shell or repository commands; evaluate hooks, helpers, or configuration; use network or credentials; mutate; run tests; load implementation governance; follow symlinks; or infer ownership from an unsupported class.

The closed registry contains exactly `ecs-task-definition-manifests`, `deployment-pipeline-manifests`, and `infrastructure-as-code`. Caller-supplied globs are prohibited. Query all three registered classes in canonical order, at most once each, with no more than 64 canonical repository-relative path names per class; reject absolute paths, parent components, controls, noncanonical names, out-of-class matches, duplicate or unsorted matches, and any report that followed symlinks. A terminal absence requires every fixed query to be complete and untruncated, no symlink encountered, no declaration conflict, every required class supported, and zero matches for every required class.
