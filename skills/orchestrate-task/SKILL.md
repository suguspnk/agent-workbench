...
After `inconclusive-delegate` and before any query results, child spawn, or ordinary routing, read the protected versioned `ownership_probe.registry_descriptor` from the already-required portable contract. Do not read router source, invent or accept caller patterns, or execute a repository command before ownership is settled. When the objective packet asserts that at least one named registered artifact class must exist in the objective-owning repository, send exactly one `awb_ownership_probe` / `awb-ownership-probe` child the unchanged descriptor. This is a path-metadata classifier, not source investigation. It uses the efficient tier at low effort, Codex read-only, and Claude `Glob` only. It must not read file contents; execute shell or repository commands; evaluate hooks, helpers, or configuration; use network or credentials; mutate; run tests; load implementation governance; follow symlinks; or infer ownership from an unsupported class.

The closed registry contains exactly `ecs-task-definition-manifests`, `deployment-pipeline-manifests`, and `infrastructure-as-code`. Query all three descriptor patterns in canonical order, at most once each, with no more than 64 canonical repository-relative path names per class. The query patterns are safe supersets: validate all paths first, then deterministically filter through the corresponding accepted-path pattern. Safe irrelevant matches are ignored; reject unsafe, duplicate, unsorted, or oversized metadata and any report that followed symlinks. A terminal absence requires every exact descriptor query to be complete and untruncated, no symlink encountered, no declaration conflict, every required class supported, and zero accepted matches for every required class.
