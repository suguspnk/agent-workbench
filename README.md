...
After activation, an ordinary pull request cannot update the trusted gate, policy, launcher, or workflow because the authoritative job compares candidate copies with the base branch. There is no preceding trusted invariant gate for the initial activation because GitHub reads a `pull_request_target` workflow from the base branch. The first landing therefore requires a separately authorized protected-base or administrator procedure: the old host-executing pull-request automation must be disabled and in-flight runs canceled before the new workflow lands. After merge, forward-test one controlled fork PR and one same-repository PR before treating the workflow as active protection. Subsequent root-control changes also require a separately reviewed protected-base procedure. Local unit tests exercise the command and failure contracts without launching Docker; this external bootstrap remains outside ordinary local development authorization.
